"""
CLIP Stage1 Text Testing Script
학습된 Text-based Additive Adapter의 Fairness 성능 평가

Usage:
    python test_stage1_txt.py --checkpoint path/to/checkpoint.pth --config config/train_stage1_txt.yaml
"""
...
def flatten_config(config):
    ...
    for section in ['model', 'text', 'dataset', 'loss', 'training', 'logging']:
...
def evaluate(model, test_loader, device, config):
    ...
    # Text anchors 초기화 확인
    if model.text_anchors is None:
        model.initialize_text_anchors()

    # Loss 함수
    criterion = CombinedLossWithText(
    ...
    # =========================================================================
    # 2. Text-Visual Alignment Metrics
    # =========================================================================
    mean_text_visual_sim = sum(text_visual_sims) / len(text_visual_sims) if text_visual_sims else 0

    print("\n" + "=" * 60)
    print("TEXT-VISUAL ALIGNMENT")
    print("=" * 60)
    print(f"  Mean Text-Visual Similarity: {mean_text_visual_sim:.4f}")

    # Text anchor 정보 출력
    model.print_text_anchor_info()

    # =========================================================================
    # 3. Fairness Metrics for Final Features (CLIP + Additive)
    # =========================================================================
    print("\n" + "=" * 60)
    ...
    # =========================================================================
    # 4. Fairness Metrics for CLIP Features (Baseline comparison)
    # =========================================================================
    print("\n" + "=" * 60)
    ...
    print(f"  Mean similarity to text anchors: {mean_text_visual_sim:.4f}")

    # =========================================================================
    # 6. Per-Subgroup Accuracy
    # =========================================================================
    print("\n" + "=" * 60)
    ...
    # =========================================================================
    # 7. Per-Subgroup Text-Visual Similarity
    # =========================================================================
    print("\n" + "=" * 60)
    print("PER-SUBGROUP TEXT-VISUAL SIMILARITY")
    print("=" * 60)

    # 각 subgroup의 visual feature와 해당 text anchor 간 similarity 계산
    final_features_norm = all_features / all_features.norm(dim=-1, keepdim=True)
    text_anchors = model.text_anchors.cpu()

    for sg_id in range(8):
        mask = (all_subgroups == sg_id)
        if mask.sum() > 0:
            sg_features = final_features_norm[mask]
            sg_anchor = text_anchors[sg_id]
            sg_sim = (sg_features * sg_anchor).sum(dim=-1).mean().item()
            print(f"  {SUBGROUP_NAMES[sg_id]}: {sg_sim:.4f} (n={mask.sum().item()})")
...
def main(args):
    ...
    print("\n" + "=" * 60)
    print("CLIP Stage1 Text Evaluation")
    print("=" * 60)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CLIP Stage1 Text Evaluation')
