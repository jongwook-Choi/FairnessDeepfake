#!/usr/bin/env python3
"""
Stage 2 Cross-dataset + Fairness 평가 스크립트
...
Usage:
    python test_stage2.py --checkpoint logs/xxx/checkpoint_best.pth
    python test_stage2.py --checkpoint logs/xxx/checkpoint_best.pth --test-datasets ff++ celebdf dfd dfdc
"""
...
class Stage2CrossDatasetTester:
    """
    Stage 2 Cross-dataset + Fairness 평가 클래스
    ...
    def __init__(self, config, checkpoint_path):
        ...
        base_result_dir = config.get('result_dir', '/workspace/code/CLIP_stage2/results')
    ...
    def load_model(self):
        ...
        # 모델 생성
        model = Stage2LinearProbingModel(
    ...
    def test_all_datasets(self, test_datasets=None):
        ...
        print("\n" + "=" * 60)
        print("Stage 2 Cross-dataset + Fairness Evaluation")
        print("=" * 60)
    ...
    def generate_report(self):
        """상세 리포트 생성"""
        print("\n" + "=" * 80)
        print("STAGE 2 CROSS-DATASET + FAIRNESS EVALUATION REPORT")
        print("=" * 80)
        print(f"Checkpoint: {os.path.basename(self.checkpoint_path)}")
...
def parse_args():
    """명령행 인자 파싱"""
    parser = argparse.ArgumentParser(description='Stage 2 Cross-dataset + Fairness Evaluation')
    ...
    parser.add_argument('--result-dir', type=str,
                        default='/workspace/code/CLIP_stage2/results',
...
def main():
    ...
    # 테스터 생성
    tester = Stage2CrossDatasetTester(config, args.checkpoint)
