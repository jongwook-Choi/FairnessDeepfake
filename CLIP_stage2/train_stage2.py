#!/usr/bin/env python3
"""
Stage 2 Linear Probing 학습 스크립트

CLIP + Additive Adapter (Stage 1 가중치, frozen) + Binary Classifier (trainable)
Deepfake Detection을 위한 Linear Probing 수행

Usage:
    python train_stage2.py --config config/train_stage2.yaml
    python train_stage2.py --config config/train_stage2.yaml --stage1-checkpoint /path/to/stage1.pth
"""
...
def parse_args():
    """명령행 인자 파싱"""
    parser = argparse.ArgumentParser(description='Stage 2 Linear Probing Training')

    parser.add_argument('--config', type=str,
                        default='config/train_stage2.yaml',
...
def print_config(config):
    """설정 출력"""
    print("\n" + "=" * 60)
    print("Stage 2 Linear Probing Configuration")
    print("=" * 60)
    ...
    # Stage 1
    print(f"\n[Stage 1]")
    print(f"  Checkpoint: {config.get('stage1_checkpoint', 'None')}")
    ...
    print(f"  Experiment: {logging_config.get('experiment_name', 'stage2')}")
