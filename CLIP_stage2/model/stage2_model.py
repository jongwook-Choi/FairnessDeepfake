"""
Stage 2 Linear Probing Model
CLIP + Additive Adapter (Stage 1 가중치 로드, frozen) + Binary Classifier (trainable)

구조:
    Input Image → CLIP Visual Encoder (Frozen) → CLIP Feature (768-dim)
                                                        ↓
                                        Additive Adapter (Frozen, Stage 1 가중치)
                                                        ↓
...
class Stage2LinearProbingModel(nn.Module):
    """
    Stage 2 Linear Probing 모델

    - CLIP + Additive Adapter (Stage 1 가중치 로드, frozen)
    - Binary Classifier (trainable)
    """
    ...
    def __init__(self,
        ...
        # Additive Adapter (Stage 1에서 가중치 로드 예정)
    ...
    def load_stage1_checkpoint(self, checkpoint_path, strict=False):
        """
        Stage 1 체크포인트에서 Additive Adapter 가중치 로드
...
class Stage2FineTuningModel(Stage2LinearProbingModel):
    """
    Stage 2 Fine-tuning 모델 (Adapter도 학습)
