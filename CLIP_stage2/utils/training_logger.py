...
class TrainingLogger:
    ...
    def _save_test_metrics_csv(self):
        ...
        fieldnames = ['epoch', 'dataset', 'auc', 'acc', 'eer', 'ap', 'num_samples', 'timestamp']

        with open(self.log_files['test_metrics_csv'], 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(self.test_metrics)
    ...
    def _save_training_summary(self, best_metric_value: float, best_metric_name: str,
        ...
        if self.test_metrics:
            summary.extend([
                "",
                "Test Dataset Results (Best Model):",
                "-" * 40
            ])

            # 최신 에포크의 테스트 결과만 (best 모델 기준)
            best_epoch_metrics = {}
            for tm in self.test_metrics:
                ds_name = tm['dataset']
                # 같은 데이터셋이면 최신 값으로 갱신
                best_epoch_metrics[ds_name] = tm

            for ds_name, tm in best_epoch_metrics.items():
                summary.append(
                    f"  {ds_name:8s}: AUC={tm['auc']:.4f}, ACC={tm['acc']:.4f}, "
